...
class mrpiracy:
	...
	def menu(self):
		...
		if not xbmcvfs.exists(controlo.controlFile):
			controlo.dialog.ok('MrPiracy', 'Devido a atualização interna do Kodi, poderão aparecer erros e estes deverão ser reportados por email.')
	...
	def getASeguir(self,id):
		lista = controlo.ler_ficheiro(os.path.join(controlo.pastaDados,'a_seguir.mrpiracy')).split(",")
		devolve = False
	...
	def episodios(self, url):
		...
		for i in resultado['data']:
			...
			try:
				...
				nome = str(nome)
			fullname = pt+br+final+semLegenda+'[COLOR '+cor+'][B]Episodio '+str(i['episodio'])+'[/B][/COLOR] '+nome
			controlo.addVideo(fullname, self.API_SITE+tipo+'.php?action=episodio&idSerie='+str(i['id_serie'])+'&idEpisodio='+str(i['id_episodio']), 'player', imagem, visto, 'episodio', i['temporada'], i['episodio'], infoLabels, self.API+i['background'])
		current = resultado['meta']['current']
		total = resultado['meta']['total']
		try: proximo = resultado['meta']['paginacao']['next']
		except: pass 
	...
	def player(self, url):
		...
		progressDialog.create('MrPiracy', 'Abrir emissão!\nPor favor aguarde...')
	...
	def download(self, url):
		...
		stream, legenda, ext_g = self.getStreamLegenda(idVideo, tipo_desc, coiso=coiso)
		if stream == False and legenda == 204:
			controlo.mensagemprogresso.close()
			controlo.alerta('MrPiracy', 'Tem de esperar 8 horas até conseguir visualizar algum video.')
			return False
	...
	def progressoTrakt(self):
		...
		for serie in json.loads(vistos):
			...
			if 'PT' in resultado['IMBD']:
				resultado['IMBD'] = re.compile('(.+?)PT').findall(resultado['IMBD'].upper())[0]
				pt = '[B][COLOR green]P[/COLOR][COLOR red]T[/COLOR]: [/B]'
	...
	def adicionarAseguir(self, url):
		...
		if 'filme' in url:
			...
			tipo = 0
			nome = resultado['nome_ingles']
			resultado = controlo.abrir_url(self.API_SITE+'aseguir.php?action=adicionar&idVideo='+id_video, header=controlo.headers, cookie=definicoes.getCookie())
		elif 'serie' in url:
			resultado = controlo.abrir_url(url, header=controlo.headers, cookie=definicoes.getCookie())
			resultado = json.loads(resultado)['data'][0]
			id_video = resultado['id_video']
			...
			resultado = controlo.abrir_url(self.API_SITE+'aseguir.php?action=adicionar&idVideo='+id_video, header=controlo.headers, cookie=definicoes.getCookie())
		resultado = json.loads(resultado)
		if resultado['a_seguir'] != "" or resultado['a_seguir'] != []:
			try:
				a_seguir = ','.join(map(str, resultado['a_seguir']))
			except:
				a_seguir = str(0)
		else:
			a_seguir = str(0)
	...
	def removerAseguir(self, url):
		...
		if 'filme' in url:
			...
			tipo = 0
			nome = resultado['nome_ingles']
			resultado = controlo.abrir_url(self.API_SITE+'aseguir.php?action=remover&idVideo='+id_video, header=controlo.headers, cookie=definicoes.getCookie())
		elif 'serie' in url:
			resultado = controlo.abrir_url(url, header=controlo.headers, cookie=definicoes.getCookie())
			resultado = json.loads(resultado)['data'][0]
			id_video = resultado['id_video']
	...
	def setFilme(self, i, vistos, opcao):
		...
		try:
			if imdbR['linguagens'].upper() == 'PT-BR':
				imdbBR = True
			elif imdbR['linguagens'].upper() == 'PT-PT':
				imdbPT = True
		except:
			pass
		...
		menuASeguir = self.getASeguir(i['id_video'])
		...
		fullname = '[COLOR '+cor+']'+pt+br+semLegenda+nome+' ('+str(i['ano'])+')[/COLOR]'
		controlo.addVideo(fullname, self.API_SITE+'filmes.php?action=id&idFilme='+str(i['id_video']), 'player', i['foto'],visto, 'filme', 0, 0, infoLabels, self.API+i['background'], trailer=imdbR['trailer'], favorito=menuFavorito, agendado=menuVerDepois, aseguir=menuASeguir)
	...
	def setSeries(self, i, vistos, opcao, tipo):
		...
		fullname = pt+br+nome+' ('+i['ano']+')'
		controlo.addDir(fullname, self.API_SITE+tipo+'.php?action=id&idSerie='+str(i['id_video']), 'temporadas', i['foto'], tipo='serie', infoLabels=infoLabels,poster=self.API+i['background'],visto=visto, menuO=True, favorito=menuFavorito, agendado=menuVerDepois, aseguir=menuASeguir)
